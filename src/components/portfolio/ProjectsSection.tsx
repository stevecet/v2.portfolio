...
export function ProjectsSection() {
  ...
  useEffect(() => {
    ...
    const fetchProjectsData = async () => {
      try {
        ...
        const uniqueCategories = [t('projects.all'), ...new Set(projectsData.projects.map(p => p.category))]
  ...
  return (
    <section id="projects" className="min-h-screen flex items-center py-20 scanlines">
                ...
                <Card className="retro-card h-full">
                  ...
                  <CardContent className="space-y-4">
                    <div className="terminal-window p-3">
                      <div className="pt-6">
                        <p className="text-muted-foreground text-sm leading-relaxed font-mono">
                          <span className="text-accent">{t('projects.description')}</span><br/>
                          {project.description}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="terminal-window p-3">
                      <div className="pt-6">
                        <div className="text-primary font-mono text-xs mb-2">
                          <span className="text-accent">{t('projects.tech')}</span>
                        </div>
                        <div className="grid grid-cols-2 gap-1">
                          {project.technologies.map((tech) => (
                            <Badge key={tech} className="retro-badge text-xs justify-center">
                              {tech}
                            </Badge>
                          ))}
                        </div>
                    ...
                    <div className="flex gap-2 pt-4">
                      {project.githubUrl && (
                        <Button variant="outline" size="sm" className="gap-2 border-primary text-primary hover:bg-primary hover:text-primary-foreground font-mono rounded-sm flex-1">
                          <Github className="h-4 w-4" />
                          {t('projects.code')}
                        </Button>
                      )}
                      {project.liveUrl && (
                        <Button size="sm" className="retro-button gap-2 rounded-sm flex-1">
